...
public class RandomOptimizer {
    ...
    protected Operator findNodeAt(Operator node, int joinNum) {
        ...
        } else if (node.getOpType() == OpType.PROJECTDUPLICATEELIMINATION) {
            return findNodeAt(((ProjectDuplicateElimination) node).getBase(), joinNum);
        } else {
            return null;
        }
    }
    ...
    public void modifySchema(Operator node) {
        ...
        if (node.getOpType() == OpType.JOIN) {
            ...
            Operator base = ((ProjectDuplicateElimination) node).getBase();
            modifySchema(base);
            Vector attrlist = ((ProjectDuplicateElimination) node).getProjAttr();
            node.setSchema(base.getSchema().subSchema(attrlist));
        }
    }
        ...
        if (node.getOpType() == OpType.JOIN) {
            ...
            switch (joinType) {
                case JoinType.STANDARDJOIN:
                    ...
                    StandardJoin sj = new StandardJoin((Join) node);
                    sj.setLeft(left);
                    sj.setRight(right);
            ...
            Operator base = makeExecPlan(((ProjectDuplicateElimination) node).getBase());
            ((ProjectDuplicateElimination) node).setBase(base);
            ((ProjectDuplicateElimination) node).setNumBuff(numbuffsort);
            return node;
        } else {
            return node;
        }
