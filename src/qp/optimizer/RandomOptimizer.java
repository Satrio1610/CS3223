...
public class RandomOptimizer {
	...
	/**
	 * implementation of Iterative Improvement Algorithm for Randomized optimization
	 * of Query Plan
	 **/
	...
	public Operator getOptimizedPlan() {
		...
		/**
		 * Randomly restart the gradient descent until the maximum specified number of
		 * random restarts (NUMITER) has satisfied
		 **/
	...
	/**
	 * Selects a random method choice for join wiht number joinNum e.g., Nested loop
	 * join, Sort-Merge Join, Hash Join etc.., returns the modified plan
	 **/
	...
	/**
	 * Applies join Commutativity for the join numbered with joinNum e.g., A X B is
	 * changed as B X A returns the modifies plan
	 **/
	...
	/**
	 * Applies join Associativity for the join numbered with joinNum e.g., (A X B) X
	 * C is changed to A X (B X C) returns the modifies plan
	 **/
	...
	protected void transformLefttoRight(Join op, Join left) {
		...
		/**
		 * CASE 1 : ( A X a1b1 B) X b4c4 C = A X a1b1 (B X b4c4 C) a1b1, b4c4 are the
		 * join conditions at that join operator
		 **/
		...
		if (leftright.getSchema().contains(leftAttr)) {
			...
			/**
			 * CASE 2: ( A X a1b1 B) X a4c4 C = B X b1a1 (A X a4c4 C) a1b1, a4c4 are the
			 * join conditions at that join operator
			 **/
	...
	protected void transformRighttoLeft(Join op, Join right) {
		...
		Join temp;
		/**
		 * CASE 3 : A X a1b1 (B X b4c4 C) = (A X a1b1 B ) X b4c4 C a1b1, b4c4 are the
		 * join conditions at that join operator
		 **/
		if (rightleft.getSchema().contains(rightAttr)) {
			...
			/**
			 * CASE 4 : A X a1c1 (B X b4c4 C) = (A X a1c1 C ) X c4b4 B a1b1, b4c4 are the
			 * join conditions at that join operator
			 **/
	...
	/**
	 * This method traverses through the query plan and returns the node mentioned
	 * by joinNum
	 **/
	...
	/**
	 * modifies the schema of operators which are modified due to selecing an
	 * alternative neighbor plan
	 **/
